...
class DashboardState:
    ...
    @staticmethod
    def _history_sep_matches(record, max_sep_deg):
    ...
    class DashboardHandler(BaseHTTPRequestHandler):
        ...
        def _history_query(self, raw_query, export=False):
            ...
            if (max_sep_deg is not None
                    and (not math.isfinite(max_sep_deg) or max_sep_deg < 0)):
                self.send_error(400, "Invalid history separation")
                return None
...
button.active{background:#173b31;color:#eef;border:1px solid #55d982}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}
...
.observer-panel{display:flex;align-items:center;flex-wrap:wrap;gap:.35rem .7rem;margin:0 0 8px;padding:6px 8px;border:1px solid #303949;border-radius:8px;font-size:.8rem}.observer-controls,.observer-status{display:flex;align-items:center;flex-wrap:wrap;gap:.3rem .55rem}.observer-controls strong{font-size:.82rem}.observer-controls button{padding:5px 9px}.observer-controls label{white-space:nowrap}.observer-controls input[type=checkbox]{accent-color:#55d982}.gps-secondary{font-size:.75rem;padding:3px 7px!important}.observer-status{min-width:0}.observer-status .arrow{color:#8793a6}.observer-ok{color:#55d982}.observer-warn{color:#f0c34d}.observer-error{color:#ff6b6b;font-weight:700}
...
function historyQuery(offset=0){let q=new URLSearchParams({offset:String(offset),limit:String(HISTORY_PAGE_SIZE),body:document.getElementById('history-body').value}),d=document.getElementById('history-date').value,s=document.getElementById('history-search').value.trim(),m=document.getElementById('history-max-sep').value.trim().replace(',','.');if(d)q.set('date',d);if(s)q.set('callsign',s);if(m)q.set('max_sep_deg',m);return q}
...
for(let id of ['history-date','history-search','history-body','history-max-sep'])document.getElementById(id).onchange=()=>loadHistory(true);
